...
public class CreateClientRequestDelegate implements WorkItemHandler {
	...
	public void executeWorkItem(WorkItem workItem, WorkItemManager manager) {
		...
		ClientRequest clientRequest = serviceDelegateHelper.createClientRequest(workItem);

		workItemResult.put(CLIENT_REQUEST, clientRequest);
		manager.completeWorkItem(workItem.getId(), workItemResult);

		logger.trace("Create Client Request  Delegate execution completed");
	}

	@Override
	public void abortWorkItem(WorkItem workItem, WorkItemManager manager) {
		manager.abortWorkItem(workItem.getId());
		logger.warn("CreateClient Request delegate Aborted");
	}
